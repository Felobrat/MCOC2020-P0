...
name = (f"timing_inv_caso_3_np.double.txt")
...
for N in NP: #recorremos el indice NP para matrices aleatorias A y B de dimension N en NP
    ...
    A = 2*(np.eye(N, dtype=np.double)) #genera la matriz A diagonal con valores 2 de dimension NxN
    for m in range(N):
        for n in range(N):
            if m==n+1:
                A[m,n]+=-1
    ...
    Ainv = inv(A, overwrite_a=True) #hace la operacion de multiplicar matrices A y B de dimension NxN*NxN
